 ...
 *
 * <quote>
 * All users of this API should employ heavy local caching. Requesting the same dataset multiple times
 * on a single day can get you banned. The same goes for request flooding.
...
public class AniDBCachedUrl {
    ...
    private byte[] getRawContents(String url) throws Exception {
        ...
        if (!cache.exists() && getMillisecondsSinceModified(cache) >= ONE_DAY_IN_MS) {
